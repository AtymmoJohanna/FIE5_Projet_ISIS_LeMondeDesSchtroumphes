function calcQtProductionforElapseTime(product: any, elapseTime: number): number {
  let qt = 0;

  if (!product.managerUnlocked) {
    // Cas sans manager : une seule production possible, pas de boucle
    ...
    // Cas avec manager : la production se relance automatiquement en boucle
    const progress = product.timeleft > 0 ? product.vitesse - product.timeleft : 0;
    const totalProgress = progress + elapseTime;
    const cycles = Math.floor(totalProgress / product.vitesse);
    const remainder = totalProgress % product.vitesse;
...
tests.forEach( t => {
    let product = { ... t.p }
    const qt = calcQtProductionforElapseTime(product, t.elapseTime)
    if (qt !== t.qt) {
